 ...
 *
 * 100 % GÉNÉRIQUE : la persona, les archétypes et le barème viennent du PROFIL
 ...
 *  Persona du cours + MOULE imposé (ADN) : le piège se conçoit DANS le moule. */
async function designTrap(a: Archetype, target: string, refImage: string | null, step: StepCb, image?: string | null, note?: string | null, statement?: string | null, mold?: MoldKind | null): Promise<DesignBrief> {
  ...
  // BLINDAGE : le modèle peut omettre un champ du schéma ; des défauts sûrs évitent
 ...
 *  MOULE imposé + few-shot RÉELS du moule (imitation resserrée) + cadrage
...
export async function architectQuestion(
  ...
  // figure émise ? Rendu SANDBOX + injection \includegraphics dans l'énoncé.
