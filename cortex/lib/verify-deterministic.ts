...
// fraction simple « a/b » — évite le détour sympy pour les cas triviaux
...
export function verifyNumericPlain(candidate: string, reference: string): DetResult {
  ...
  // Resserré : c'est le DERNIER nombre du candidat (position de la réponse finale)
...
export async function verifyDeterministic(
  ...
  if (!cand || !ref) {
    // cas « figure » : la RÉFÉRENCE est la vérité de la figure (figureTruth),
  ...
  // type « code » : le candidat est un PROGRAMME, la preuve = exécution
  ...
  // type « figure » : la question se résout DEPUIS une figure générée par un
