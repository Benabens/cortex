...
import { DEFAULT_COURSE, tryGetCourse } from "@/lib/courses";
...
// cache des profils génériques construits à la volée (onboarding zéro-code).
const GENERIC_CACHE = new Map<string, CourseProfile>();

/**
 * Profil d'un cours. Un cours ENREGISTRÉ (table `courses`) sans profil curaté
...
export function getProfile(courseId?: string): CourseProfile {
  ...
  if (hit) return hit;
  // le profil curaté d'un cours est désormais une COLONNE (`profile_id`),
  // plus un id de cours : un cours créé depuis l'interface peut donc réutiliser un
  // profil existant, ou n'en avoir aucun (→ générique, dérivé de l'ADN des annales).
  const c = tryGetCourse(id);
  const curated = c?.profileId ? PROFILES[c.profileId] : undefined;
  if (curated) return curated;
  if (id && id !== DEFAULT_COURSE && c) {
