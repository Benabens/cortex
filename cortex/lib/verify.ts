...
function buildVerifyPrompt(spec: ExamSpec): string {
  ...
  return [
    ...
    `   - "ok" : énoncé bien posé ET corrigé correct et complet.`,
    `   - "wrong" : le corrigé proposé est FAUX ou incomplet (valeurs, étapes, ou complexité). Donne alors corrected_solution_tex = LE BON corrigé en LaTeX.`,
    `   - "ambiguous" : l'énoncé est mal posé / non résoluble / ambigu / incohérent avec son barème. Explique dans issue.`,
    `3) Marque violates_exclusion=true si l'énoncé porte sur un sujet EXCLU par les directives ci-dessus.`,
    `Sois STRICT : un corrigé faux est pire que rien. Si tu hésites entre ok et wrong, choisis wrong et fournis ta version.`,
