...
/**
 * Exercices « Labs » au format de la question Projet (Q6) du Final 2025.
 *
...
export const LABS: LabDef[] = [
  {
    ...
    files: [
      ...
      { rel: path.join(LABS_ROOT, "lab1_entrainement", "ex2.c"), role: "entropie avec 4 bugs (entraînement de l'étudiant)", key: true },
  ...
  },
  {
    ...
    files: [
      ...
      { rel: path.join(GC, "done", "lab2", "Step2", "get_file.c"), role: "client : demande un fichier, le reçoit (solution de l'étudiant)", key: true },
  ...
  },
  {
    ...
    files: [
      ...
      { rel: path.join(GC, "done", "Lab4", "src", "inode.c"), role: "inode_read/findsector (direct/indirect) — solution de l'étudiant", key: true },
  ...
  },
  {
    ...
    files: [
      { rel: path.join(GC, "done", "lab5", "get_file.c"), role: "serveur TCP multi-threadé COMPLET (workers, table de mutex) — solution de l'étudiant", key: true },
...
export async function generateLabExercise(
  ...
  step("Génération de l'exercice Labs — moule Q6 2025 + vrai code du lab (LLM)…", 25);
...
/** La série figée : un exo par lab, avec liens PDF durables (UI /entrainement). */
