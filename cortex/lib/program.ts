...
/**
 * « Programme & Maîtrise ». PAR COURS, 100 % additif.
 *
 *  - `topics`  : taxonomie typée + pondérée des exercices de la matière (dérivée des VRAIS
 *                finals via le LLM + vision, complétée par les séries / le plan du cours).
...
export type Topic = {
  ...
  label: string;
  method: string | null;
  exoType: string | null; // format réel de l'exo
  trap: string | null; // piège typique
  ...
  chapterId: number | null; // chapitre du plan de cours où la notion est traitée (null = non rattaché)
};

/** Une occurrence RÉELLE d'une notion dans un final (pour l'accordéon : toutes visibles au dépli). */
...
export type TopicView = Topic & {
  ...
  // ── Enrichissement depuis l'index exo-par-exo (exam_exercises). Absent (null/0) tant
  ...
  // ── complétude accordéon : chapitre rattaché + TOUTES les occurrences (chaque final + page) ──
...
export async function ensureProgramSchema(): Promise<void> {
  ...
  // blueprint approfondi : type d'exo (format réel) + piège type, ajoutés au fil de l'eau.
  await q.ensureColumns("topics", ["exo_type", "trap"]);
  await q.ensureTable("mastery");
}

// ---------------- Analyseur de blueprint ----------------
 ...
 * profil d'examen pondéré, complété par les séries et le plan du cours. Une passe LLM (+ vision
...
export async function analyzeBlueprint(opts: { onStep?: (m: string, p: number) => void } = {}): Promise<AnalyzeResult> {
  ...
  const p = profile();
  // Les archétypes du cours = la liste des types que le GÉNÉRATEUR sait produire. On demande au LLM
  ...
  const archetypeList = p.archetypes
    ...
    .join("\n");
  // Vision : si le cours a des images-étalon (vrais finals rendus), le LLM les REGARDE.
  ...
  step("Classification des exercices par type via le LLM (+ vision)…", 35);
...
// ---------------- Agrégation de la partition DEPUIS l'index exo-par-exo ----------------
...
/**
 * Reconstruit `topics` À PARTIR de l'index `exam_exercises` : regroupe les exos par type
 * (clustering LLM des labels bruts ; repli déterministe par label normalisé), `exam_count` = compte
...
export async function aggregateTopicsFromIndex(opts: { onStep?: (m: string, p: number) => void } = {}): Promise<{ count: number; exercises: number }> {
  ...
  // clustering LLM (résilient → repli : chaque label distinct = un type)
  ...
  if (rawLabels.length > 1) {
    ...
    } catch (e) { step(`Clustering LLM indisponible (${(e as Error).message.slice(0, 40)}) → repli par label`, 95); }
  }
  ...
  // dé-doublonnage « à la source » : retire les notions EMPILÉES (analyses passées, FR/EN,
...
/**
 * Construit le blueprint EXHAUSTIF : index exo-par-exo des finals → agrégation en partition
...
export async function rebuildBlueprintFromIndex(opts: { onStep?: (m: string, p: number) => void } = {}): Promise<{ exams: number; exercises: number; types: number; chapters: number; mapped: number }> {
  ...
  // plan de cours du prof + rattachement des notions (repli propre si non dérivable).
  ...
  // ADN d'examen (moules + figures + texture) détecté depuis les annales.
...
/** TOUTES les occurrences réelles par topic_id (chaque final + page), pour l'accordéon. */
...
/** Chapitre du plan enrichi des compteurs (tête d'accordéon : nb notions / nb tombées). */
...
export type ProgramOverview = {
  topics: TopicView[];
  chapters: PlanChapterView[]; // plan de cours du prof (vide si non dérivable pour ce cours)
...
// ---------------- Enrichissement par-topic (deep-links + ordre de cours + points réels) ----------------
...
export async function programOverview(): Promise<ProgramOverview> {
  ...
  // plan de cours du prof + compteurs de tête de chapitre (dérivés des notions rattachées).
...
// ---------------- Boucle score 0-10 → courbe de l'oubli ----------------
...
/** Parcours « couvrir tout le programme » : le type le plus LOURD encore sous le seuil. */
...
export function topicTarget(t: Topic): string {
  // la cible de génération porte la méthode, le FORMAT réel de l'exo et le piège typique
