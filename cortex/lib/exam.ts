...
export type ExamQuestion = {
  ...
  difficulty?: number;
  points?: number;
  // figure rendue (PNG dans examsDir, déjà injectée dans statement_tex) + valeurs
...
/** Brief de génération destiné au LLM : tout le contexte + le format JSON attendu. */
...
// ---------------- EXERCICE CIBLÉ : 1 exo qualité examen, sans garde ----------------
...
export async function generateTargetedExercise(
  ...
  // le pipeline ARCHITECTE multi-passes (conception du piège → rédaction →
  ...
  // routage par matière. Entrées : IMAGE, ou TEXTE riche classé. La voie mono-passe
  ...
  if (target || imageRel) {
    try {
      ...
      // Texte : détecter le type. Court → sujet direct (pas d'appel). Riche → classification.
...
export function buildBatchPrompt(ctx: Ctx, slots: { category: string; points: number; brief: string; mold?: string | null }[]): string {
  const p = profile();
  // calibrage de difficulté (cs-202) : menu de pièges réels + style prof + distribution.
  const diff = currentCourse() === DEFAULT_COURSE ? difficultyBlockForExam() : "";
  return [
    p.directivesBlock(),
    ``,
    ...
    // le MOULE du slot (ADN détecté) est imposé quand il existe ; un slot sans
...
async function generateBatch(ctx: Ctx, slots: Slot[]): Promise<ExamQuestion[]> {
  ...
  // le LLM → permet de prouver la RÉSILIENCE du pipeline de façon reproductible et gratuite.
...
/**
 * GÉNÉRATION EN LOTS : 2 lots de 3 exos en parallèle (timeouts courts),
...
export async function generateExamViaClaudeCode(opts: { verify?: boolean; count?: number; focus?: string; onStep?: StepCb; jobId?: number } = {}): Promise<{ id: number; url: string; texError?: string }> {
  ...
  // composeur (CS-202 calcul/trace) : l'utilisateur peut choisir le NOMBRE d'exercices. Défaut (count absent)
  ...
  // « Mets l'accent sur… » (focus générique) — force 1-2 slots à porter sur le thème choisi
  ...
  // (stub de test : vérifier des questions factices n'a aucun sens et appellerait le vrai LLM)
  ...
  // RÉSILIENCE : on découpe TOUS les slots en lots (longueur réelle, pas plafonnée à 6).
  ...
  // quand le run appartient à un JOB (opts.jobId), la progression est persistée
  ...
  const saveCkpt = async (batchIndex: number) => {
    ...
    // persistance d'un lot → preuve déterministe de la reprise sans perte.
  ...
  async function architectBatch(bslots: Slot[], lotIdx: number): Promise<ExamQuestion[]> {
      ...
      while (idx < bslots.length) {
        ...
        try {
          const r = await architectQuestion(a, a.concept, slot.points, {
            maxRounds: 1,
            mold: (slot.mold as import("@/lib/molds").MoldKind | null) ?? null, // moule du slot (ADN)
    ...
    // RÉSILIENCE : on RENVOIE ce qu'on a (même partiel/vide). Un lot incomplet ne fait
  ...
  /**
   * PIPELINE : chaque lot est vérifié+durci DÈS qu'il est généré, sans attendre
   ...
   * maxAttempts de durcissement 3→2 (validé : cap du pire cas sans perte mesurée).
   */
  const settled = await Promise.all(
    batches.map(async (bslots, i) => {
      // un lot ne fait JAMAIS échouer le job : tout throw inattendu est rattrapé ici et
      ...
      try {
        ...
        if ((process.env.CORTEX_TEST_FAIL_BATCHES ?? "").split(",").filter(Boolean).includes(String(i)))
          throw new Error("timeout simulé (test de résilience)");
        let qs: ExamQuestion[];
  ...
  // on n'échoue QUE si rien n'a abouti (tous les lots ont timeout/échoué). Sinon l'examen
...
/** Prompt pour le provider CLI (headless) : le brief complet + sortie JSON stricte. */
