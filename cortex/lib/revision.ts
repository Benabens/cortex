...
// ─────────────────────── Index exhaustif question-par-question ───────────────────────
...
export async function assignLectureRanks(opts: { onStep?: StepCb } = {}): Promise<number> {
  ...
  // Approche robuste : on demande au LLM d'ORDONNER les topics selon le déroulé du cours (titres de
...
// ─────────────────────── Parcours généré (couverture 100 % à la bonne proportion) ───────────────────────
...
// ─────────────────────── EXTENSION massive du parcours (≥10 QCM/topic, ciblé faiblesses) ───────────────────────

/**
 * TAXONOMIE DE TOPICS au grain « THÈME » (≈25), dans l'ordre du cours du prof (cf. carte du cours
 * de l'étudiant). Chaque thème : un `target` de QCM (sur-pondéré sur ses thèmes faibles), des `aliases`
