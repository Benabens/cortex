/**
 * SETUP D'UN COURS EN UNE COMMANDE (idempotent, cold-start).
 *
 ...
 * « chaud » d'un conteneur. (b)/(c) ont besoin du fournisseur LLM ; s'il manque, l'ingestion
...
import { DEFAULT_COURSE, ensureCoursesLoaded, normalizeCourse } from "../lib/courses";
...
// Résolu dans main(), APRÈS ensureCoursesLoaded() : en postgres, aucun cours
// n'est connu avant le chargement du cache (cf. lib/courses).
...
async function main() {
  COURSE = normalizeCourse(argVal("course"));
  ...
  if (!llmAvailable()) {
    console.log(`\n⚠ CLI « claude » introuvable → étapes (b) format et (c) blueprint SAUTÉES.`);
    console.log(`  L'ingestion a réussi (refs : ${refsN}). Lance « claude » une fois (authentification), puis :`);
  ...
  // (c) BLUEPRINT EXHAUSTIF — index exo-par-exo des finals → partition agrégée. Non-fatale.
...
ensureCoursesLoaded().then(main).catch((e) => {
