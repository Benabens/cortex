...
async function main() {
  ...
  const b0 = bankStats(), p0 = planStats();
  ...
  for (const t of themeCoverage().sort((a, b) => a.rank - b.rank || a.label.localeCompare(b.label)))
    console.log(`    L${t.rank} ${t.label}${t.weak ? " ⚑" : ""} : ${t.qcm} QCM / ${t.open} ouv. (cible ${t.target})${t.qcm >= 10 ? " ✓" : " ✗<10"}`);
}

main().catch((e) => { console.error("Échec revision:extend :", e); process.exit(1); });
