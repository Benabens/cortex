...
import { coursePaths, ensureCoursesLoaded, listCourseIds } from "../lib/courses";
...
function parseArgs(): Args {
  ...
  const all = listCourseIds().filter((id) => fs.existsSync(coursePaths(id).dbPath));
  return {
    user: get("--user") ?? "owner",
...
async function migrateCourse(user: string, courseId: string, drop: boolean): Promise<boolean> {
...
(async () => {
  ...
  // Les cours vivent en base : charger le cache avant de lister les cours par défaut.
  ...
  for (const course of ARGS.courses) {
    try {
      ok = (await migrateCourse(ARGS.user, course, ARGS.drop)) && ok;
    } catch (e) {
