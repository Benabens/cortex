...
import { q } from "../db/q";
import { ensureCoursesLoaded, listCourses, normalizeCourse } from "@/lib/courses";
...
async function main() {
  ...
  // SEUILS BLOQUANTS : l'éval devient un garde-fou chiffré, pas un
...
ensureCoursesLoaded().then(main).catch((e) => { console.error("Échec eval :", e); process.exit(1); });
