/**
 * Billing (déploiement v1) — tests hermétiques, sans réseau ni Max :
 *  - mapping économe : « opus » → Sonnet sur le provider anthropic sauf opt-in ;
 *  - estimation de coût par modèle (tarifs 07/2026) ;
...
test("estimateCostUsd : tarifs par préfixe, inconnu → tarif conservateur (opus)", () => {
  ...
  assert.ok(Math.abs(billing.estimateCostUsd("claude-sonnet-5", 1_000_000, 1_000_000) - 18) < 1e-9);
  // opus : 5 + 25
...
test("recordUsage : claude-code (€0) n'écrit RIEN ; anthropic écrit coût + user + cours", async () => {
  ...
  // 0,2 MTok × 3 + 0,1 MTok × 15 = 2,1 $
  assert.ok(Math.abs(Number(row.cost_usd) - 2.1) < 1e-9);
});
...
test("totalSpendUsd : somme lue en DB (source de vérité cross-process)", async () => {
  ...
  assert.ok(Math.abs(total - 2.1) < 1e-9);
});
